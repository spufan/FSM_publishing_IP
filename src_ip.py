...
class Remediation:
    ...
    # enforce on FSM itsealf (extracting source IP address from incident file and add it to the .html file list)
    def detect_enforce_on(self, incident_xml):
        srcIpAddr=self.get_incident_attribute(incident_xml,"incidentSource", "srcIpAddr")
        ...
        for line in f:
            index += 1
            if srcIpAddr in line:
                flag = 1
                break
        if flag == 0:
            print('Source IP address', srcIpAddr, 'Not Found')
            f = open(listpathandname, "a")
            f.write(srcIpAddr)
            f.write("\n")
            f.close()
        else:
            print('IP address', srcIpAddr, 'Found In List', index)
        f.close()
